...
export default function TableList({ data = [], type }: TableListProps) {
  ...
  const renderPagination = () => (
    <div className="mt-8 mb-8 flex items-center justify-between">
      <span className="text-base font-medium text-[#364c84]">
        Page <span className="font-semibold text-[#364c84]">{page}</span> of <span className="font-semibold text-[#364c84]">{totalPages}</span>
      </span>
      <div className="flex items-center gap-3">
        <button
          onClick={goPrev}
          disabled={page === 1}
          className="rounded-lg bg-[#e7f1a8] px-4 py-2 text-base font-medium text-[#364c84] transition-all hover:bg-[#d4e894] hover:shadow-md disabled:cursor-not-allowed disabled:bg-gray-100"
        >
          Prev
        </button>
        <span className="text-base text-[#364c84]">  
  ...
  const renderSorter = () => (
    <div className="mb-6 flex justify-end">
      <label className="flex items-center gap-3">
        <span className="text-base font-semibold text-[#364c84]">Sort by:</span>
        <select
          value={sortBy}
    ...
    return (
      <div>
        ...
        <table className="w-full border-collapse border border-gray-400">
          <thead>
            <tr className="bg-[#e7f1a8] text-[#364c84]">
              <th className="border border-gray-400 p-2">Title</th>
              <th className="border border-gray-400 p-2">Date Released</th>
              <th className="border border-gray-400 p-2">Genre</th>
          ...
          <tbody>
            {pagedData.map((item, index) => (
              <tr key={index} className="text-[#364c84]">
                ...
                <td className="border border-gray-400 p-2 text-center">
                  <button
                    ...
                    className="rounded bg-[#85b1ee] px-2 py-2 text-[#fffdf5] text-sm font-semibold disabled:bg-gray-300 disabled:cursor-not-allowed"
                  >
